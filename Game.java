...
public class Game {
	...
	public Game() {
		...
		if (winners.size() == 1) {
			...
			System.out.println("Winner: " + winner.getBestHand() + " " + winner.getBestHand().getHandType().toString()
					+ " " + winner.getName() + " (Balance: $ " + winner.getChips() + ")");
		} else {
			System.out.println("Winners: ");
			for (Player winner : winners) {
				winner.setChips(winner.getChips() + dealer.getPot() / winners.size());
				System.out.println(winner.getBestHand() + " " + winner.getBestHand().getHandType().toString() + " "
						+ winner.getName() + " (Balance: $ " + winner.getChips() + ")");
		...
		while (playAgain.equals("y")) {
			...
			for (Player p : dealer.getPlayers()) {
				...
				System.out.println();
				System.out.println("Best Hand " + p.getBestHand().toString() + " " + " ("
						+ p.getBestHand().getHandType().toString() + ")");
			}
			...
			if (winners.size() == 1) {
				Player winner = winners.get(0);
				System.out
						.println("Winner: " + winner.getBestHand() + " " + winner.getBestHand().getHandType().toString()
								+ " " + winner.getName() + " (Balance: $ " + winner.getChips() + ")");
			} else {
				System.out.println("Winners: ");
				for (Player winner : winners) {
